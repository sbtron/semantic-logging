...
##Create a windows VM and display diagnostics data on ELK 

Here is a sample VM + ELK template which sets up a windows VM with diagnostics turned on and then creates a VM running the ELK stack which is configured to pull in diagnostics data generated by the windows VM.
